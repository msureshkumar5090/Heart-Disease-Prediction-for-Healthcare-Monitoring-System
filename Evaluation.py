...
def evaluation_1(sp, act):
    ...
    F1_score = (2 * tp) / (2 * tp + fp + fn) * 100
    MCC = ((tp * tn) - (fp * fn)) / math.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn))
    EVAL = [tp, tn, fp, fn, accuracy, sensitivity, FNR, specificity,  FPR, precision, FDR, NPV, F1_score, MCC]
    return EVAL
...
def net_evaluation(sp, act):
    ...
    MCC = ((tp * tn) - (fp * fn)) / math.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn))
    EVAL = [tp, tn, fp, fn, Dice, Jaccard, accuracy, IoU, sensitivity, specificity, precision, FPR, FNR, NPV, FDR, F1_score,
            MCC]
    return EVAL
