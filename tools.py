...
def check_spectrum(spectrum):
    ...
    # if len(spectrum) == 1:
    #     return np.zeros(1, dtype=np.float32).reshape(-1, 2)
    # spectrum = np.asarray(spectrum, dtype=np.float32, order="C")
    # if spectrum.ndim != 2:
...
def clean_spectrum(spectrum,
    ...
    #     spectrum = spectrum[spectrum[:] <= max_mz]

    # 2. Centroid peaks
    spectrum = spectrum[np.argsort(spectrum[:])]
    ...
    if noise_removal is not None and spectrum.shape[0] > 0:
        ...
        spectrum = spectrum[spectrum[:] >= max_intensity * noise_removal]
...
def centroid_spec(spec, ms2_ppm=None, ms2_da=None):
    ...
    if need_centroid:
        intensity_order = np.argsort(-spec[:])
        spec_new = []
        for i in intensity_order:
            if ms2_da is None:
                if ms2_ppm is None:
                    ...
                    mz_delta_allowed = ms2_ppm * 1e-6 * spec[i]
            else:
            ...
            if spec[i] > 0:
                ...
                i_left = i - 1
                while i_left >= 0:
                    mz_delta_left = spec[i] - spec[i_left, 0]
